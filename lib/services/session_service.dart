...
class SessionService {
  ...
  Future<void> saveNavIndex(int index) =>
      _prefs.setInt(_Keys.navIndex, index);
  ...
  Future<void> saveScanHistory(List<ScanHistoryModel> history) async {
    ...
    final encoded = limited.map((s) =>
        '${s.id}|${s.projetTitre}|${s.localisation}|${s.date}'
    ).toList();
  ...
  List<ScanHistoryModel> loadScanHistory() {
    ...
    }).whereType<ScanHistoryModel>().toList();
  }
}
